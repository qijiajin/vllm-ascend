...
#include <cstdio>
#include <cmath>
#include "ai_infra_fused_infer_attention_sink_metadata.h"
#include "../../common/aicpu/cpu_context_util.h"
#include "ai_infra_fused_infer_attention_sink_metadata_aicpu.h"

using namespace optiling;

namespace aicpu {
namespace {
    const char* const AI_INFRA_FUSED_INFER_ATTENTION_SINK_METADATA = "AiInfraFusedInferAttentionSinkMetadata";
}  // namespace

uint32_t AiInfraFusedInferAttentionSinkMetadataKernel::Compute(CpuKernelContext &ctx)
{
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::GenMetaData(const SplitResult &splitRes)
{
...
void AiInfraFusedInferAttentionSinkMetadataKernel::CalcInnerSizeMla(uint32_t seqSize)
{
    /*
    ...
    */
    if (batchInvariant_) {
        ...
        */
        ...
        if (splitKVFlag_ && inputLayout_ != "TND") {
            if (seqSize == 256U) {
                sInnerSize_ = 128U;
...
void AiInfraFusedInferAttentionSinkMetadataKernel::CalcMBaseSizeMla()
{
    if (batchInvariant_) {
...
void AiInfraFusedInferAttentionSinkMetadataKernel::CreateSplitInputMla(BaseInfo &baseInfo, SplitParam &splitParam) const
{
    // 构造分核输入参数
    ...
    baseInfo.isS1G = inputLayout_ == "TND" || inputLayout_ == "BSH_BSND";
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::DealSameSeqEachBatch()
{
...
void AiInfraFusedInferAttentionSinkMetadataKernel::CalcInnerSizeGqa(uint32_t s2Size)
{
    /*
     * CANN reduceSum函数在规约长度S  S>=512、S<512两种场景内部实现的累加顺序不同，
     * 为了保障batchInvariant场景计算结果的一致性，将算子在S2方向的基本块大小固定为256，
    ...
    */
    if (batchInvariant_) {
        ...
        if (inputLayout_ == "TND" || inputLayout_ == "NTD") {
...
void AiInfraFusedInferAttentionSinkMetadataKernel::CalcMBaseSizeGqa()
{
    if (inputLayout_ == "TND" || inputLayout_ == "NTD") {
...
void AiInfraFusedInferAttentionSinkMetadataKernel::CreateSplitInputGqa(BaseInfo &baseInfo, SplitParam &splitParam) const
{
    // 构造分核输入参数
    baseInfo.bSize = bSize;
    baseInfo.n2Size = n2Size;
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::Prepare(CpuKernelContext &ctx)
{
    // input
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::ParamsParse()
{
    return (CheckSingleParam() && ParseAxisInfo());
}

bool AiInfraFusedInferAttentionSinkMetadataKernel::CheckSingleParam()
{
    // 1. 基础输出校验
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::CheckSingleParaInputLayout()
{
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::ParseAxisInfo()
{
    return (CheckSingleParaNumHeads() && GetBatchSize() && GetS1Size() && GetS2Size() && GetPreNextToken() &&
            GetActualSeqInfo());
}

bool AiInfraFusedInferAttentionSinkMetadataKernel::CheckSingleParaNumHeads()
{
    if (numHeadsQ_ <= 0) {
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::GetBatchSize()
{
    // 获取B基准值
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::GetS1Size()
{
    // 获取S1基准值
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::GetActualSeqLenSize(uint32_t &size)
{
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::GetActualSeqKVLenSize(uint32_t &size)
{
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::GetS2Size()
{
    ...
    if (inputLayoutKv_ == "TND" || inputLayoutKv_ == "NTD") {
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::GetPreNextToken()
{
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::GetActualSeqInfo()
{
    maxActualseq = s2Size;
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::ParamsCheck()
{
    return (CheckFeature() && CheckMultiParaConsistency());
}

void AiInfraFusedInferAttentionSinkMetadataKernel::CheckIsMla()
{
    // 仅支持ROPE分开传输
    if (ropeHeadDim_ == 0U) {
        isMlaSink = false;
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::CheckFeature()
{
    return (CheckFeatureActualSeqLensQData() && CheckFeatureActualSeqLensKvData() && CheckFeatureAxisInfo());
}

bool AiInfraFusedInferAttentionSinkMetadataKernel::CheckFeatureAxisInfo() const
{
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::CheckFeatureActualSeqLensQData()
{
    if (actualSeqLengths_ == nullptr) {
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::CheckFeatureActualSeqLensKvData()
{
    if (actualSeqLengthsKv_ == nullptr) {
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::CheckMultiParaConsistency()
{
    return (CheckActualSeqLensQ() && CheckActualSeqLensKv() && CheckParamSinkNumber() && CheckSinkNumber());
}

bool AiInfraFusedInferAttentionSinkMetadataKernel::CheckParamSinkNumber()
{
    if (kSinkNum_ < 0) {
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::CheckSinkNumber()
{
    ...
    if (sinkNum_ > 0 && inputLayout_ != "TND") {
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::CheckMlaSinkNumber()
{
    // D=128时，走GQA模板
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::CheckActualSeqLensQ() const
{
    if ((actualSeqLengths_ == nullptr) || (actualSeqLengths_->GetData() == nullptr)) {
        return true;
    }
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::CheckActualSeqLensKv() const
{
    if ((actualSeqLengthsKv_ == nullptr) || (actualSeqLengthsKv_->GetData() == nullptr)) {
        return true;
    }
...
uint32_t AiInfraFusedInferAttentionSinkMetadataKernel::GetS1SeqSize(uint32_t bIdx, const BaseInfo &baseInfo)
{
...
uint32_t AiInfraFusedInferAttentionSinkMetadataKernel::GetS2SeqSize(uint32_t bIdx, const BaseInfo &baseInfo)
{
...
int64_t AiInfraFusedInferAttentionSinkMetadataKernel::CalcPreTokenLeftUp(uint32_t s1Size,
                                                                         uint32_t s2Size,
...
int64_t AiInfraFusedInferAttentionSinkMetadataKernel::CalcNextTokenLeftUp(uint32_t s1Size,
                                                                          uint32_t s2Size,
                                                                          const BaseInfo &baseInfo)
{
    auto mode = static_cast<SparseMode>(baseInfo.sparseMode);
...
int64_t AiInfraFusedInferAttentionSinkMetadataKernel::CalcCost(uint32_t basicM, uint32_t basicS2)
{
...
BlockCost<int64_t> AiInfraFusedInferAttentionSinkMetadataKernel::CalcCostTable(uint32_t s1NormalSize,
...
Range<uint32_t> AiInfraFusedInferAttentionSinkMetadataKernel::CalcS2Range(uint32_t s1GIdx,
                                                                          const BaseInfo &baseInfo,
...
void AiInfraFusedInferAttentionSinkMetadataKernel::CalcSplitInfo(SplitContext &splitContext)
{
...
void AiInfraFusedInferAttentionSinkMetadataKernel::CalcBatchCache(uint32_t bIdx,
...
void AiInfraFusedInferAttentionSinkMetadataKernel::CalcS1GCacheWithSinkNumber(uint32_t s1GIdx,
                                                                              const SplitContext &splitContext,
                                                                              const BatchCache &batchCache,
                                                                              S1GCache &s1GCache,
                                                                              uint32_t sinkNumber)
...
void AiInfraFusedInferAttentionSinkMetadataKernel::CalcS1GCache(uint32_t s1GIdx,
                                                                const SplitContext &splitContext,
                                                                const BatchCache &batchCache,
                                                                S1GCache &s1GCache)
{
...
void AiInfraFusedInferAttentionSinkMetadataKernel::CopyTmpResult(SplitResult &tmpRes, SplitResult &splitRes)
{
...
void AiInfraFusedInferAttentionSinkMetadataKernel::ClearTmpResult(SplitResult &tmpResult)
{
...
void AiInfraFusedInferAttentionSinkMetadataKernel::CalcBatchCost(uint32_t bIdx,
                                                                 const SplitContext &splitContext,
                                                                 CostInfo &costInfo)
{
    const BaseInfo &baseInfo = splitContext.baseInfo;
...
void AiInfraFusedInferAttentionSinkMetadataKernel::CalcCostInfo(SplitContext &splitContext)
{
    const BaseInfo &baseInfo = splitContext.baseInfo;
    const SplitInfo &splitInfo = splitContext.splitInfo;
...
void AiInfraFusedInferAttentionSinkMetadataKernel::UpdateCursor(const SplitContext &splitContext,
...
void AiInfraFusedInferAttentionSinkMetadataKernel::AssignByBatch(const SplitContext &splitContext,
                                                                 AssignContext &assignContext)
{
...
void AiInfraFusedInferAttentionSinkMetadataKernel::AssignByRow(const SplitContext &splitContext,
                                                               AssignContext &assignContext)
{
    if (assignContext.isFinished) {
        return;
...
void AiInfraFusedInferAttentionSinkMetadataKernel::UpdateCurS2IdxWithSinkNumber(const SplitContext &splitContext,
                                                                                AssignContext &assignContext,
                                                                                uint32_t sinkNumber)
{
    const BaseInfo &baseInfo = splitContext.baseInfo;
...
void AiInfraFusedInferAttentionSinkMetadataKernel::AssignByBlock(const SplitContext &splitContext,
                                                                 AssignContext &assignContext)
{
    if (assignContext.isFinished) {
        return;
...
void AiInfraFusedInferAttentionSinkMetadataKernel::ForceAssign(const SplitContext &splitContext,
                                                               AssignContext &assignContext)
{
    if (assignContext.isFinished) {
        return;
...
bool AiInfraFusedInferAttentionSinkMetadataKernel::IsNeedRecordFDInfo(const AssignContext &assignContext,
...
void AiInfraFusedInferAttentionSinkMetadataKernel::RecordFDInfo(const SplitContext &splitContext,
...
void AiInfraFusedInferAttentionSinkMetadataKernel::CalcSplitPlan(uint32_t coreNum,
                                                                 int64_t costLimit,
                                                                 const SplitContext &splitContext,
                                                                 SplitResult &result)
{
...
void AiInfraFusedInferAttentionSinkMetadataKernel::SplitFD(SplitResult &result)
{
...
void AiInfraFusedInferAttentionSinkMetadataKernel::SplitCore(uint32_t coreNum,
                                                             const BaseInfo &baseInfo,
